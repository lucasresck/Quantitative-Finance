  ...
  \section*{Problema 3.18 \cite{carmona_rene_2014}}
    ...
    \begin{enumerate}
      ...
      \item[2, 3.] Chamemos $\rho$ o coeficiente de correlação \textit{Pearson} entre $X \sim \textrm{Lognormal}(0, 1)$ e $Y \sim \textrm{Lognormal}(0, \sigma^2)$. Então queremos calcular:
        ...
        Então vamos calcular cada um desses termos. Se $Z_1 = \log X$ e $Z_2 = \log Y$, então $\mathbf{Z} = (Z_1, Z_2)$ têm distribuição normal multivariada. Vejamos a \textit{moment-generating function} da distribuição normal multivariada:
        \begin{align*}
        ...
        De maneira análoga, é possível calcular $\E\{X\} = e^{\frac{1}{2}}$ e $\E\{Y\} = e^{\frac{\sigma^2}{2}}$. Encontramos também $\var\{X\} = (e - 1)e$ e $\var\{Y\} = (e^{\sigma^2} - 1)e^{\sigma^2}$. Então encontramos $\rho$:
...
SIG2 = seq(from = 0.1, to = 200, length.out = 100)
