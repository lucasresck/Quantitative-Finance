...
\begin{document}
  ...
  \section*{Problema 1.1}
    ...
    \begin{enumerate}
      ...
      \item Em um GG-plot, com $\pi_p^{(1)}$ na abscissa e $\pi_p^{(2)}$ na ordenada, no lado esquerdo do gráfico teremos pontos abaixo de $y=x$. Ou seja, para uma mesma área, $F_2$ "leva em quantis maiores (em valor absoluto)", portanto possui cauda esquerda mais pesada.
